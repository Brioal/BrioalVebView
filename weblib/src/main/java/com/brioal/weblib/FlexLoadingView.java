...
public class FlexLoadingView extends android.support.v7.widget.AppCompatImageView {
    ...
    private void closeAnimation() {
        ValueAnimator animator = ValueAnimator.ofFloat(mFlexOffset, mFlexOffset / 4 );
    ...
    private void startAnimation() {
        ValueAnimator animator = ValueAnimator.ofFloat(mFlexOffset /4, mFlexOffset);
        animator.setDuration(300);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mLinesCount > 2) {
            ...
            for (int i = 0; i <mLinesCount; i++) {
                //方块内容
                RectF rect = new RectF(singleLineWidth * (2 * i+1), mCurrentFlexOffset, singleLineWidth * (2 * i + 2), mHeight - mCurrentFlexOffset);
